...
import com.BiliClient.Noctilucere.activity.video.BuiltinPlayerActivity;
...
public class PlayerApi {
    ...
    public static void jumpToPlayer(Context context, String videourl, String danmakuurl, String title, boolean local){
        ...
        switch (SharedPreferencesUtil.getString("player", "null")) {
            ...
            case "builtin":
                // 新 API 适配（Noctilucere 芋泥P）：内置播放器，无需安装第三方播放器
                intent.setClass(context, BuiltinPlayerActivity.class);
                intent.putExtra("url", videourl);
                intent.putExtra("title", title);
                intent.putExtra("local", local);
                intent.putExtra("danmakuurl", danmakuurl);
                break;
